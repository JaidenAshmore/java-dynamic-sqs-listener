...
public class SendMessageTracingExecutionInterceptorIntegrationTest {
    ...
    public void spanInformationIsSharedOverSqsMessageAttributes() throws Exception {
        ...
        // act
        final String messageId = sqsAsyncClient
            .sendMessage(builder -> builder.queueUrl(queueUrl).messageBody("body"))
            .get(5, TimeUnit.SECONDS)
            .messageId();

        final ReceiveMessageResponse receiveMessageResponse = sqsAsyncClient
            .receiveMessage(builder -> builder.queueUrl(queueUrl).waitTimeSeconds(5).messageAttributeNames("b3"))
            .get(5, TimeUnit.SECONDS);
        scopedSpan.finish();
        ...
        final TraceContextOrSamplingFlags traceContextOrSamplingFlags = SendMessageRemoteGetter
            .create(tracing)
            .extract(message.messageAttributes());
        final MutableSpan sendMessageSpan = testSpanHandler.spans().get(0);
        final MutableSpan testSpan = testSpanHandler.spans().get(1);
        assertThat(traceContextOrSamplingFlags.context().traceIdString()).isEqualTo(sendMessageSpan.traceId());
