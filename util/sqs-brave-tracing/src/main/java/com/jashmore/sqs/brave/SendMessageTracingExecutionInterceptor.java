    ...
    public void afterExecution(final Context.AfterExecution context, final ExecutionAttributes executionAttributes) {
        ...
        try {
            ...
            if (context.httpResponse().isSuccessful()) {
                spanDecorator.decorateMessageSpanOnSuccess(request, response, context.httpResponse(), span);
            } else {
                spanDecorator.decorateMessageSpanOnFailure(request, response, context.httpResponse(), span);
