...
public class ProducerTwoApplication {
    ...
    public AvroSchemaRegistrySqsAsyncClient avroSchemaRegistrySqsAsyncClient(
        ...
        final List<Resource> schemaImports = Optional
            .ofNullable(avroMessageConverterProperties.getSchemaImports())
            .map(Arrays::asList)
            .map(Collections::unmodifiableList)
            .orElse(Collections.emptyList());
        final List<Resource> schemaLocations = Optional
            .ofNullable(avroMessageConverterProperties.getSchemaLocations())
            .map(Arrays::asList)
            .map(Collections::unmodifiableList)
            .orElse(Collections.emptyList());

        return new AvroSchemaRegistrySqsAsyncClient(
            delegate,
